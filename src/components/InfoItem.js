DocMail = require('../componentsUI/DocMail.js')
...
const InfoItem = () => {
    ...
    if(mailObject?.authorName?.avatar) {
        authorImg.src = mailObject?.authorName?.avatar
    } else {
        authorImg.src = 'https://www.tenstickers.co.uk/wall-stickers/img/preview/one-love-rasta-wall-sticker-10261.png'
    }

    // режем под формат hh:mm
    const getHours = mailObject?.date?.toString().slice(11, 13)
    const getMinutes = mailObject?.date?.toString().slice(14, 16)
    const dateDay = mailObject?.date?.toString().slice(8, 10)
    let dateMonth = mailObject?.date?.toString().slice(5, 7)
    ...
    if(mailObject?.categFlag) {
        if(mailObject?.categFlag === "Заказы") {
        ...
        } else if(mailObject?.categFlag === "Финансы") {
        ...
        } else if(mailObject?.categFlag === "Регистрации") {
        ...
        } else if(mailObject?.categFlag === "Путешествия") {
        ...
        } else if(mailObject?.categFlag === "Билеты") {
        ...
        } else if(mailObject?.categFlag === "Штрафы и налоги") {
    ...
    for(let i = 0; i < mailObject?.to?.length; i++) {
        recipients.unshift(` ${mailObject.to[i].name} ${mailObject.to[i].surname}`)
    }

    recipients.join(', ')

    title.innerHTML = `${mailObject?.title}`
    name.innerHTML = `${mailObject?.authorName?.name}` + ' ' + `${mailObject?.authorName?.surname}`
    date.innerHTML = `${dateDay} ${dateMonth}, ${getHours}:${getMinutes}`
    receiver.innerHTML = `Кому: ${recipients}`
    mailText.innerHTML = `${mailObject?.text}`
    infoDocAmo.innerHTML = `1 файл`
    infoDocSize.innerHTML = `(5мб)`
    infoDocLoad.innerHTML = `Скачать `
    categoriesMailText.innerHTML = `${mailObject?.categFlag}`
